...
public class ErrorBinder implements ErrorBinderSpigot {

    @Override
    public void error(CommandSender commandSender, Throwable throwable, String s) {
        if (throwable instanceof IllegalArgumentException) {
            commandSender.sendMessage(ChatAction.of(LobbyPlugin.getInstance().getPrefix()).with("§c" + throwable.getMessage()).comp());
        } else {
            commandSender.sendMessage(ChatAction.of(LobbyPlugin.getInstance().getPrefix()).with("§cAn error occurred while executing the command!").comp());
